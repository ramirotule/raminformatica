...
function Sidebar({
  ...
  const items: { id: AdminSection; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "home", label: "Slides Home", icon: <Home size={16} /> },
    {
      id: "productos",
      label: dict.admin.productos,
    ...
    try {
      ...
      for (const p of (productsData as any[]) || []) {
        ...
        if (sellPrice !== undefined && sellPrice !== null) {
          ...
          const exactAllowed = cost / 0.9 + 25;
      ...
      <div className="card" style={{ padding: 24, marginBottom: 32 }}>
        ...
        >
          <div>
            ...
            >
              Verifica que los productos activos cumplan: Precio Venta ≥
              Redondeado(5, (Costo / 0.90) + $25)
            </p>
          </div>
          <button
          ...
          <div style={{ marginTop: 24 }}>
            {priceIssues.length === 0 ? (
              <div
                ...
                style={{ display: "flex", alignItems: "center", gap: 8 }}
              >
                <CheckCircle2 size={16} /> Todos los productos cumplen con la
                regla de precio.
              </div>
            ) : (
              <div>
                ...
                >
                  <AlertCircle size={16} /> Se encontraron {priceIssues.length}{" "}
                  producto(s) por debajo del precio mínimo esperado.
                </div>
                <div
      ...
      <div className="card" style={{ padding: 24, marginBottom: 32 }}>
        ...
        >
          <div>
            ...
            >
              Encuentra automáticamente productos con nombres idénticos o con
              una similitud del 95% o superior.
            </p>
          </div>
          <button
          ...
          <div style={{ marginTop: 24 }}>
            {duplicateGroups.length === 0 ? (
              <div
                ...
                style={{ display: "flex", alignItems: "center", gap: 8 }}
              >
                <CheckCircle2 size={16} /> No se encontraron productos
                duplicados.
              </div>
            ) : (
              <div>
                ...
                >
                  <AlertCircle size={16} /> Se encontraron{" "}
                  {duplicateGroups.length} grupo(s) de posibles productos
                  duplicados.
                </div>
                <div
                          ...
                          >
                            ...
                            >
                              ...
                              <div>
                                ...
                                >
                                  {p.categories?.name || "Sin categoría"} • ID:{" "}
                                  {p.id.substring(0, 8)}...
                                </div>
                              </div>
                            </div>
        ...
        >
          <div>
            ...
            <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem" }}>
              Te recomendamos configurar la autenticación de dos factores (2FA)
...
function AdminProductos() {
    ...
    try {
      const [pRes, cRes, bRes, provRes] = await Promise.all([
        supabase
          .from("products")
          .select(
            "*, categories(*), brands(*), product_variants(*, prices(*), inventory(*)), product_images(*)",
          )
  ...
  const filteredProducts = (() => {
    ...
    return list.filter((p) => {
      const matchesProvider =
        !providerFilter || p.provider_id === providerFilter;
      ...
      return (
        matchesProvider && matchesCategory && matchesBrand && matchesStatus
      );
    });
  })();
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    if (sortField === "name") {
      ...
      const priceA = a.product_variants?.[0]?.prices?.find(
        (pr: any) => pr.currency === "USD",
      )?.amount;
      const priceB = b.product_variants?.[0]?.prices?.find(
        (pr: any) => pr.currency === "USD",
      )?.amount;
    ...
    try {
      ...
      for (const p of productsToClean) {
        ...
        if (mode === "clean") {
          ...
          const escapedSearch = bulkRenameSearch.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&",
          );
          const regex = new RegExp(escapedSearch, "gi");
          newName = p.name
            .replace(regex, bulkRenameReplace)
            .replace(/\s+/g, " ")
            .trim();
        }
  ...
  async function bulkStatus(active: boolean) {
    ...
    showAlert(
      "success",
      `Productos ${active ? "activados" : "pausados"} masivamente.`,
    );
    setSelectedIds(new Set());
    load();
  }
  ...
  async function performBulkCategory() {
    ...
    await ((supabase as any).from("products") as any)
      .update({ category_id: bulkCategoryForm })
      .in("id", Array.from(selectedIds));
    showAlert(
      "success",
      "Categoría actualizada para los productos seleccionados.",
    );
  ...
  async function performBulkProvider() {
    ...
    await ((supabase as any).from("products") as any)
      .update({ provider_id: bulkProviderForm })
      .in("id", Array.from(selectedIds));
    showAlert(
      "success",
      "Proveedor actualizado para los productos seleccionados.",
    );
  ...
  async function handleBulkEnrichment(
    mode: "all" | "descriptions" | "images" = "all",
  ) {
    if (!selectedIds.size) return;
    setIsSaving(true);
    try {
    ...
    try {
      ...
      if (editProduct) {
        ...
        const { error: pErr } = await (supabase as any)
          ...
          .update({
            ...
            condition: form.condition as "new",
        ...
        if (variant) {
          const { error: vErr } = await (
            (supabase as any).from("product_variants") as any
          )
            .update({
              sku: form.sku || `${slug}-v1`,
            ...
            .upsert(
              {
                ...
                currency: "USD",
                amount: parseFloat(form.priceUSD),
              },
      ...
      if (currentProductId) {
        const currentExistingIds = new Set(
          images.filter((i) => i.isExisting).map((i) => i.id),
        );
        const removedImages = (editProduct?.product_images || []).filter(
          (img: any) => !currentExistingIds.has(img.id),
        );
        ...
        for (const rmImg of removedImages) {
          ...
          if (
            rmImg.storage_path &&
            rmImg.storage_path.startsWith(currentProductId)
          ) {
      ...
      if (imageErrors) {
        showAlert(
          "error",
          `Guardado con errores en imágenes. Revisa el bucket "Images". Detalles: ${imageErrorDetails.join(", ")}`,
        );
      } else {
        showAlert(
          "success",
    ...
    if (error) {
      ...
      if (error.message.includes('column "is_featured" does not exist')) {
        ...
        const { error: error2 } = await (supabase as any)
          .from("products")
          .update({ featured: !p.is_featured })
          .eq("id", p.id);
        if (error2) console.error("Error with featured column too:", error2);
        else {
          showAlert("success", 'Actualizado usando columna "featured"');
          load();
        }
      }
    } else {
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button
              ...
              onClick={() => handleBulkEnrichment("descriptions")}
              disabled={isSaving}
            >
              {isSaving ? (
                <Loader2
            ...
            <button
              ...
              onClick={() => handleBulkEnrichment("images")}
              disabled={isSaving}
            >
              {isSaving ? (
                <Loader2
          ...
          <table>
            <thead>
              <tr>
                ...
                <th
                  onClick={() => handleSort("providers.name")}
                  style={{ cursor: "pointer", userSelect: "none" }}
                >
                  <div
                    style={{ display: "flex", alignItems: "center", gap: 4 }}
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      style={{
                        fontWeight: 600,
                        color: (p as any).cost_price
                          ? "var(--text-secondary)"
                          : "var(--text-muted)",
                      }}
                    >
                      {(p as any).cost_price ? (
                        formatUSD(Number((p as any).cost_price))
                      ) : (
                        <span style={{ color: "var(--text-muted)" }}>—</span>
                      )}
                    </td>
          ...
          <div className="modal">
            ...
            >
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="form-cost-price">
                  Precio Costo (USD)
                </label>
                <input
                  id="form-cost-price"
                  ...
                  onChange={(e) => {
                    ...
                    setForm((f) => ({
                      ...
                      cost_price: cost,
                      priceUSD:
                        finalPrice > 0 ? finalPrice.toString() : f.priceUSD,
                    }));
                  }}
                  placeholder="0.00"
                />
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="form-provider">
                  Proveedor
                </label>
                <SearchableSelect
                  id="form-provider"
              ...
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                ...
                >
                  Palabras clave que ayudarán a encontrar el producto más fácil
                  (ej: "PS5" por "Playstation").
                </p>
              </div>
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        border: "1px solid var(--border)",
                        background: showOriginalDesc
                          ? "var(--primary-light)"
                          : "var(--bg-secondary)",
                        color: showOriginalDesc
                          ? "var(--primary)"
                          : "var(--text-muted)",
                        cursor: "pointer",
                  ...
                  {showOriginalDesc ? (
                    <textarea
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          descripcion_original: e.target.value,
                        }))
                      }
                      style={{
              ...
              >
                  ...
                  >
                    <Plus
                      ...
                      style={{
                        ...
                        transform: showMoreDetails ? "rotate(45deg)" : "none",
                      }}
                    />
                ...
                {showMoreDetails && (
                  ...
                  >
                    ...
                    <div className="form-group">
                      <label className="form-label" htmlFor="form-connectivity">
                        {dict.admin.conectividad}
                      </label>
                      <input
                        id="form-connectivity"
                        className="form-input"
                    ...
                    >
                      <label className="form-label" htmlFor="form-long-desc">
                    ...
                    >
                      <label className="form-label" htmlFor="form-desc">
                        {dict.admin.descripcion} (Texto completo inferior)
                      </label>
                      <textarea
                        id="form-desc"
                        className="form-textarea"
              ...
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                ...
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  * Si subes archivos, necesitas el bucket "Images". Arrastra
                  para ordenar (la 1ra es la portada).
                </div>
                    ...
                    {images.map((img, idx) => (
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          if (
                            (e.target as HTMLElement).closest(".btn-delete-img")
                          )
                            return;
              ...
              >
                ...
                <button
                  onClick={() => {
                    ...
                    const newImgList = images.filter(
                      (i) => i.id !== idToRemove,
                    );
                    setImages(newImgList);
                    if (newImgList.length === 0) setLightboxOpen(false);
                    else if (lightboxIndex >= newImgList.length)
                      setLightboxIndex(newImgList.length - 1);
                  }}
        ...
        >
          <div style={{ textAlign: "center", color: "#fff" }}>
            <Loader2
              className="animate-spin"
              size={48}
              style={{ marginBottom: 16, color: "var(--accent-light)" }}
            />
            <h2 style={{ fontSize: "1.25rem", fontWeight: 800 }}>
              Guardando producto...
            </h2>
            <p style={{ color: "rgba(255,255,255,0.6)", marginTop: 8 }}>
              Subiendo imágenes y procesando datos, por favor espera.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="modal" style={{ maxWidth: 400, overflow: "visible" }}>
            ...
            <div className="form-group" style={{ zIndex: 10 }}>
              <label className="form-label">
                Selecciona la nueva categoría para los {selectedIds.size}{" "}
                productos:
              </label>
              <SearchableSelect
            ...
            >
              ...
              <button
                className="btn btn-primary"
                onClick={performBulkCategory}
                disabled={!bulkCategoryForm}
              >
                Guardar
              </button>
            </div>
        ...
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 400, overflow: "visible" }}>
            <div className="modal-title">
              ...
              <button
                onClick={() => setBulkBrandOpen(false)}
                className="btn btn-ghost btn-sm"
              >
                <X size={16} />
              </button>
            ...
            <div className="form-group" style={{ zIndex: 10 }}>
              <label className="form-label">
                Selecciona la nueva marca para los {selectedIds.size} productos:
              </label>
              <SearchableSelect
                value={bulkBrandForm}
            ...
            >
              ...
              <button
                className="btn btn-primary"
                onClick={performBulkBrand}
                disabled={!bulkBrandForm}
              >
                Guardar
              </button>
            </div>
        ...
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 400, overflow: "visible" }}>
            <div className="modal-title">
              ...
              <button
                onClick={() => setBulkProviderOpen(false)}
                className="btn btn-ghost btn-sm"
              >
                <X size={16} />
              </button>
            ...
            <div className="form-group" style={{ zIndex: 10 }}>
              <label className="form-label">
                Selecciona el nuevo proveedor para los {selectedIds.size}{" "}
                productos:
              </label>
              <SearchableSelect
            ...
            >
              ...
              <button
                className="btn btn-primary"
                onClick={performBulkProvider}
                disabled={!bulkProviderForm}
              >
                Guardar
              </button>
            </div>
        ...
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 800, width: "90vw" }}>
            <div className="modal-title">
              ...
              <button
                onClick={() => setBulkImagesOpen(false)}
                className="btn btn-ghost btn-sm"
              >
                <X size={16} />
              </button>
            ...
            >
              ...
              >
                <strong>Tip Pro:</strong> Cargá fotos acá para añadirlas a{" "}
                <strong>TODOS</strong> los productos seleccionados.
              </p>
              <input
                type="file"
                multiple
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <label
                        htmlFor={`bulk-file-${id}`}
                        className="btn btn-ghost btn-sm"
                        style={{ padding: 6, minWidth: 0, borderRadius: "50%" }}
                      >
                        <Plus size={16} />
                      </label>
                    </div>
            ...
            >
              ...
              <button
                ...
                disabled={Object.keys(bulkImagesData).every(
                  (k) => (bulkImagesData[k]?.length || 0) === 0,
                )}
              >
        ...
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 450 }}>
            ...
            <div style={{ padding: "16px 0" }}>
              ...
              >
                Aplicando cambios a <strong>{selectedIds.size}</strong>{" "}
              ...
              >
                ...
                >
                  Opción 2: Limpieza y Capitalización
                </h4>
                <p
                  style={{
                    fontSize: "0.8rem",
                ...
                >
                  Elimina emojis, normaliza espacios y aplica formato título
                  (Ej: "SMARTWATCH 64GB" → "Smartwatch 64GB").
                </p>
                <button
        ...
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 450 }}>
            ...
            <div className="modal-body">
              ...
              >
                Los tags ingresados se <b>agregarán</b> a los que ya tengan los
                productos seleccionados. Separe con comas.
              </p>
              <div className="form-group">
            ...
            >
              ...
              <button
                className="btn btn-primary"
                onClick={performBulkTags}
                disabled={!bulkTagsForm.trim() || isSaving}
              >
          ...
          <div className="modal" style={{ maxWidth: 440 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              <p style={{ marginBottom: 12 }}>
                Vas a eliminar <strong>{selectedIds.size} producto(s)</strong>{" "}
                seleccionado(s).
              </p>
              <p
                style={{
      ...
      {singleDeleteConfirm && (
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 440 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              <p>
                ¿Estás seguro de que deseás eliminar{" "}
                <strong>"{singleDeleteConfirm.name}"</strong>?
              </p>
              <p
                style={{
                  color: "var(--red)",
            ...
            >
              ...
              <button className="btn btn-danger" onClick={executeSingleDelete}>
                <Trash2 size={15} /> Confirmar Eliminación
              </button>
            </div>
          </div>
...
function AdminCategorias() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      let error;
      if (editId) {
        ({ error } = await ((supabase as any).from("categories") as any)
          .update(data)
          .eq("id", editId));
      } else {
      ...
      showAlert(
        "success",
        editId ? "Categoría actualizada." : "Categoría creada.",
      );
  ...
  async function executeDelete() {
    ...
    if (count > 0) {
      showAlert(
        "error",
        `No se puede eliminar: la categoría tiene ${count} producto(s) asociado(s). Reasignálos antes de borrarla.`,
      );
          ...
          <div className="modal" style={{ maxWidth: 450 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              {deleteConfirm.count > 0 ? (
                <div>
                  <p style={{ marginBottom: 12 }}>
                    La categoría <strong>"{deleteConfirm.name}"</strong> tiene{" "}
                    <strong>{deleteConfirm.count} producto(s)</strong>{" "}
                    asociado(s).
                  </p>
                  <p style={{ color: "var(--red)", fontWeight: 500 }}>
                    Reasigná o eliminá esos productos antes de borrar la
                    categoría.
                  </p>
                </div>
              ) : (
      ...
      {bulkDeleteConfirm && (
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 440 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              <p style={{ marginBottom: 12 }}>
                Vas a eliminar <strong>{selectedIds.size} categoría(s)</strong>{" "}
                seleccionada(s).
              </p>
              <p
                style={{
...
function AdminMarcas() {
  ...
  const executeBulkDelete = async () => {
    ...
    if (blocked > 0)
      showAlert(
        "error",
        `${blocked} marca(s) no se eliminaron porque tienen productos asociados.`,
      );
    else showAlert("success", `${selectedIds.size} marca(s) eliminada(s).`);
    setIsSaving(false);
    load();
  };

  const [sortField, setSortField] = useState<"name" | "slug" | "created_at">(
    "name",
  );
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");
  ...
  async function handleSave() {
    ...
    try {
      ...
      let error;
      if (editId) {
        ({ error } = await ((supabase as any).from("brands") as any)
          .update(data)
          .eq("id", editId));
      } else {
  ...
  async function executeDelete() {
    ...
    if (count > 0) {
      showAlert(
        "error",
        `No se puede eliminar: la marca tiene ${count} producto(s) asociado(s). Reasignálos antes de borrarla.`,
      );
      setDeleteConfirm(null);
      return;
    }
      ...
      {modalOpen && (
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 400 }}>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="brand-logo">
                Logo
              </label>
              <input
                type="file"
          ...
          <div className="modal" style={{ maxWidth: 450 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              {deleteConfirm.count > 0 ? (
                <div>
                  <p style={{ marginBottom: 12 }}>
                    La marca <strong>"{deleteConfirm.name}"</strong> tiene{" "}
                    <strong>{deleteConfirm.count} producto(s)</strong>{" "}
                    asociado(s).
                  </p>
                  <p style={{ color: "var(--red)", fontWeight: 500 }}>
                ...
                <p>
                  ¿Estás seguro de que deseás eliminar la marca{" "}
                  <strong>"{deleteConfirm.name}"</strong>?
                </p>
              )}
            </div>
      ...
      {bulkDeleteConfirm && (
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 440 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              <p style={{ marginBottom: 12 }}>
                Vas a eliminar <strong>{selectedIds.size} marca(s)</strong>{" "}
                seleccionada(s).
              </p>
              <p
                style={{
              ...
              >
                Esta acción no se puede deshacer. Los productos asociados a
                estas marcas pueden verse afectados.
              </p>
            </div>
            <div
              style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}
...
function AdminInventario() {
  const [items, setItems] = useState<
    (Inventory & { product_variants: ProductVariant & { products: Product } })[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [alert, setAlert] = useState<{
    type: "success" | "error";
  ...
  const [sortField, setSortField] = useState<
    "product" | "sku" | "available" | "reserved" | "status"
  >("product");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");

  const handleSort = (
    field: "product" | "sku" | "available" | "reserved" | "status",
  ) => {
    if (sortField === field) {
      setSortOrder(sortOrder === "asc" ? "desc" : "asc");
    } else {
  ...
  const sortedItems = [...items].sort((a, b) => {
    ...
    if (sortField === "product") {
      ...
      const getStatusRank = (inv: any) => {
        ...
        const isLow =
          inv.qty_available > 0 && inv.qty_available <= inv.low_stock_threshold;
  ...
  async function saveQty(id: string) {
    const { error } = await ((supabase as any).from("inventory") as any)
      .update({
        qty_available: parseInt(qty),
        updated_at: new Date().toISOString(),
      })
      .eq("id", id);
    if (error) showAlert("error", error.message);
    else {
      showAlert("success", "Stock actualizado.");
      setEditItem(null);
      load();
    }
  }
  ...
  return (
    <div>
      ...
      >
        <h1 style={{ fontSize: "1.5rem", fontWeight: 800 }}>
          {dict.admin.inventario}
        </h1>
        <button
          id="admin-refresh-inventory"
          className="btn btn-ghost btn-sm"
          onClick={load}
        >
          <RefreshCw size={15} />
                  ...
                  <tr key={inv.id}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => {
                          setEditItem(inv.id);
                          setQty(String(inv.qty_available));
                        }}
                        title="Editar stock"
                      >
                        <Pencil size={14} />
...
function AdminHome() {
  ...
  async function saveLogo() {
    ...
    try {
      ...
      if (editItem) {
        await ((supabase as any).from("brand_logos") as any)
          .update(payload)
          .eq("id", editItem.id);
      } else {
  ...
  async function saveNews() {
    ...
    try {
      ...
      if (newsFile) {
        ...
        const { error: uploadError } = await supabase.storage
          .from("Images")
          .upload(fileName, newsFile, { upsert: true });
  ...
  async function deleteNews(id: string, name: string) {
    ...
    if (error) showAlert("error", error.message);
      ...
      showAlert("success", "Novedad eliminada.");
      loadAll();
    }
  }

  async function updateNewsOrder(items: WeeklyNews[]) {
    ...
    if (error) {
      ...
      showAlert("error", "Error al guardar el nuevo orden.");
    } else {
        ...
        <div>
          ...
          >
            <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>
              {logos.length} logo(s) configurados
            </p>
            <button className="btn btn-primary btn-sm" onClick={openNewLogo}>
              <Plus size={14} /> Nuevo Logo
            </button>
          </div>
          <div
            style={{
                ...
                {newsItems.map((it, idx) => (
                  <tr
                    ...
                    onDrop={async (e) => {
                      ...
                      // Persistir en DB
                      await updateNewsOrder(updatedItems);
                    }}
                    style={{ cursor: "grab" }}
                  >
                    <td
                    ...
                    <td>
                      <span
                        className={`badge ${it.active ? "badge-new" : "badge-used"}`}
                      >
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button
                          className="btn btn-ghost btn-sm"
                          onClick={() => openEditNews(it)}
                        >
                          <Pencil size={14} />
                        </button>
                        <button
                          className="btn btn-danger btn-sm"
                          onClick={() => deleteNews(it.id, it.title)}
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
        ...
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 500 }}>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setLogoForm((f) => ({ ...f, active: e.target.checked }))
                      }
                      style={{
                        width: 18,
                        height: 18,
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Image size={16} />{" "}
                      {newsPreview ? "Cambiar Imagen" : "Seleccionar Imagen"}
                    </button>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewsForm({ ...newsForm, link_url: e.target.value })
                    }
                  ...
                  >
                    Puedes usar rutas relativas como /productos?q=termino o
                    enlaces externos.
                  </span>
                </div>
                  ...
                  >
                    <div className="form-group">
                      ...
                      <input
                        ...
                        placeholder="Ej: Banner iPhone 16"
                      />
                    </div>
                    <div className="form-group">
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setNewsForm({ ...newsForm, active: e.target.checked })
                        }
                        style={{
                          width: 18,
                          height: 18,
            ...
            >
              ...
              <button
                className="btn btn-primary"
                onClick={tab === "brands" ? saveLogo : saveNews}
                disabled={isSaving}
              >
                {isSaving ? (
                  <Loader2 className="animate-spin" size={15} />
                ...
                Guardar {tab === "brands" ? "Logo" : "Novedad"}
              </button>
            </div>
          </div>
        </div>
...
function AdminProveedores() {
  ...
  const bulkStatus = async (active: boolean) => {
    ...
    showAlert(
      "success",
      `${selectedIds.size} proveedor(es) ${active ? "activado(s)" : "desactivado(s)"}.`,
    );
    setIsSaving(false);
    load();
  };
  ...
  const [sortField, setSortField] = useState<"name" | "email" | "created_at">(
    "name",
  );
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");
  ...
  async function handleSave() {
    ...
    try {
      ...
      let error;
      if (editId) {
        ({ error } = await ((supabase as any).from("providers") as any)
          .update(data)
          .eq("id", editId));
      } else {
      ...
      showAlert(
        "success",
        editId ? "Proveedor actualizado." : "Proveedor creado.",
      );
      setModalOpen(false);
      load();
    } catch (error: any) {
      ...
      {bulkDeleteConfirm && (
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 440 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              <p style={{ marginBottom: 12 }}>
                Vas a eliminar <strong>{selectedIds.size} proveedor(es)</strong>{" "}
                seleccionado(s).
              </p>
              <p
                style={{
      ...
      {singleDeleteConfirm && (
        <div className="modal-overlay animate-fade-in-fast">
          <div className="modal" style={{ maxWidth: 440 }}>
            ...
            <div style={{ padding: "10px 0 20px 0" }}>
              <p>
                ¿Estás seguro de que deseás eliminar al proveedor{" "}
                <strong>"{singleDeleteConfirm.name}"</strong>?
              </p>
              <p
                style={{
...
export default function AdminClient() {
  ...
  return (
    <div className="admin-layout" style={{ paddingTop: 0 }}>
      ...
      <div className="admin-content">{sectionMap[section]}</div>
    </div>
  );
}
